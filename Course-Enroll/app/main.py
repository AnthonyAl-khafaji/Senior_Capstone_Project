...
class StudentCatalogCourse(BaseModel):
    course_id: int
    course_name: str
    department: str
    credits: int
...
@app.get("/student/catalog", response_model=List[StudentCatalogCourse])
def student_catalog(
    ...
    return crud.get_student_catalog_courses(
        session,
...
@app.put("/admin/courses/{course_id}/update-instructor", response_model=Course)
def update_assigned_instructor(
    course_id: int,
    payload: AssignInstructorPayload,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
    ...
    require_role(user, [Role.ADMIN])
    existing_course = session.get(Course, course_id)
    if not existing_course:
        raise HTTPException(status_code=404, detail="course not found")

    if existing_course.instructor_id is None:
        raise HTTPException(
            status_code=400,
            detail="course does not have an assigned instructor yet",
        )
...
@app.put("/admin/courses/{course_id}/schedule", response_model=Course)
def update_schedule(
    course_id: int,
    ...
    if not (
        existing_course.schedule_days
        and existing_course.schedule_time
        and existing_course.classroom_location
    ):
        raise HTTPException(
            status_code=400,
            detail="schedule not created yet",
        )

    if (
        payload.schedule_days is None
        and payload.schedule_time is None
        and payload.classroom_location is None
    ):
        raise HTTPException(
            status_code=400,
            detail="provide at least one schedule field to update",
        )

    course = crud.set_course_schedule(
        session,
        course_id=course_id,
        schedule_days=payload.schedule_days or existing_course.schedule_days,
        schedule_time=payload.schedule_time or existing_course.schedule_time,
        classroom_location=payload.classroom_location or existing_course.classroom_location,
    )
    if not course:
        raise HTTPException(status_code=404, detail="course not found")
    return course
...
@app.delete("/admin/courses/{course_id}/schedule", response_model=Course)
def delete_schedule(
    course_id: int,
    user: User = Depends(get_current_user),
    session: Session = Depends(get_session),
    ...
    if not (course.schedule_days or course.schedule_time or course.classroom_location):
        raise HTTPException(status_code=400, detail="schedule not found")

    cleared = crud.clear_course_schedule(session, course_id=course_id)
    if not cleared:
        raise HTTPException(status_code=404, detail="course not found")
    return cleared
...
def instructor_dashboard(
    ...
    return {
        "courses": crud.get_instructor_dashboard_data(session, instructor.instructor_id),
    }
