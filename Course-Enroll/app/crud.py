...
def get_student_catalog_courses(session: Session, *, include_unavailable: bool = False):
    courses = get_courses(session)
    rows = []

    for course in courses:
        enrolled_count = len(
            session.exec(select(Enrollment).where(Enrollment.course_id == course.course_id)).all()
        )
        seats_remaining = max(course.capacity - enrolled_count, 0)
        is_available = seats_remaining > 0
