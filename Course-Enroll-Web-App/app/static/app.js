...
async function loadStudentCatalog() {
  ...
  courses.forEach((c) => {
    ...
    tr.innerHTML = `
      ...
      <td><button data-course-id="${c.course_id}">Enroll</button></td>
    `;
...
async function loadInstructorCourses() {
  try {
    ...
    const select = $("instructor-roster-course");
    ul.innerHTML = "";
    select.innerHTML = "";
...
async function loadInstructorRoster() {
  const courseId = $("instructor-roster-course").value;
  if (!courseId) {
    msg("Select a course for roster", false);
    return;
  }
  try {
    const roster = await api(`/instructor/courses/${courseId}/roster`);
    const ul = $("instructor-roster");
    ul.innerHTML = "";
    roster.forEach((row) => {
      const li = document.createElement("li");
      li.textContent = `#${row.student_id} ${row.name} (${row.email}) - ${row.grade_status}`;
      ul.appendChild(li);
    });
  } catch (e) {
    msg(`Roster load failed: ${e.message}`, false);
  }
}
...
function bindInstructorActions() {
  $("refresh-instructor-courses").onclick = loadInstructorCourses;
  $("load-roster").onclick = loadInstructorRoster;
  ...
  $("save-grade").onclick = async () => {
    ...
    try {
      ...
      msg(`Grade save failed: ${e.message}`, false);
    }
  };

  $("delete-grade").onclick = async () => {
  ...
  $("save-schedule").onclick = async () => {
    ...
    try {
      await api(`/admin/courses/${courseId}/schedule`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ schedule_days, schedule_time, classroom_location }),
      });
